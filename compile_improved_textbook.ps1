...
pandoc $allFiles -o $outputFile --template=improved_template.latex --pdf-engine=xelatex -V fontsize=11pt --toc --toc-depth=2 --number-sections --top-level-division=chapter -V toc-title="Table of Contents" --from=markdown-smart 2>&1 | Out-File -FilePath "pandoc_compile.log"
